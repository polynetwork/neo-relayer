...
func (this *SyncService) GetCurrentRelayChainSyncHeight(neoChainID uint64) (uint32, error) {
	...
	if err := neoConsensusPeer.Deserialization(mccommon.NewZeroCopySource(value)); err != nil {
...
func (this *SyncService) syncProofToRelay(key string, height uint32) error {
	...
	if res.Result.StateHeight < height {
		return fmt.Errorf("[syncProofToRelay] neoSdk.GetStateHeight error: State height not synced")
	}
	...
	log.Infof("[syncProofToRelay] txHash is: %s", txHash.ToHexString())
	return nil
}
